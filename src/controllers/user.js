    ...
    (err, existingUser) => {
      ...
      if (!existingUser) {
        ...
        existingUser.comparePassword(req.body.password, (err, isMatch) => {
          ...
          if (isMatch) {
            var access_token = utils.createAccessToken(
              existingUser.email,
              existingUser._id,
            );
      ...
      if (!existingUser) {
        ...
        User.findOne(
          ...
          function(err, userWithTokens) {
            ...
            User.update(
              { _id: existingUser._id, 'jwttokens.access_token': access_token },
              {
                $set: { 'jwttokens.$.enabled': false },
              },
              function(err) {
                if (err) {
                  return res.status(HttpStatus.CONFLICT).send({
...
exports.postSignup = (req, res, next) => {
  ...
  var access_token = utils.createAccessToken(user.email, user._id);
  var id_token = utils.createIdToken({
    email: user.email,
  });
